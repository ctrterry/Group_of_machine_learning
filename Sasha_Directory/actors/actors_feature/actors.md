...
SELECT 
    ...
    career_span,
    recent_avg_rating,
    is_rating_std_dev_missing,
    is_recent_avg_rating_missing,
    actor_score
FROM read_csv_auto('C:/Users/sasaa/OneDrive/Documents/GOLANG/src/MyVault/NOTES/UC-Davis/S25/ECS171/project_imdb_rating/Group_of_machine_learning/actors/actor_scores_and_features.csv', delim=',', header=true);
```

## MOVIES WITH ACTOR SCORES
```sql
CREATE TABLE movies_with_actor_scores AS
SELECT 
    m.tconst, m.primaryTitle, m.startYear, m.averageRating, m.numVotes,
    COALESCE(AVG(a.actor_score), 5.0) AS movie_actor_score
FROM movies_with_genres m
LEFT JOIN title_principals p ON m.tconst = p.tconst
LEFT JOIN actor_scores a ON p.nconst = a.nconst
WHERE p.category IN ('actor', 'actress') AND p.ordering <= 7
